...
class JsonMessage(BaseMessage):
    ...
    def raise_exception_if_optional_field_is_a_negative_number(self, tag):
        ...
        if val:
            if not (type(val) == float or type(val) == int) or val < 0:
                raise InvalidMessageFieldException(
                    self.raw_message, self.message, tag, val
                )
    ...
    def __init__(self, raw_message):
        ...
        if self.type == "0":  # Heartbeat
            ...
            if self.message.get("BrokerID") == 4:
                raise InvalidMessageFieldException(
                    self.raw_message, self.message, "Broker", "FOXBIT"
                )
            ...
            if "Token" in self.message:
                if token is None or len(token.strip()) == 0:
                    raise InvalidMessageFieldException(
                        self.raw_message, self.message, "Token", ""
                    )
            ...
            if (
                "DepositMethodID" not in self.message
                and "DepositID" not in self.message
                and "Currency" not in self.message
            ):
                raise InvalidMessageMissingTagException(
                    self.raw_message, self.message, "DepositID,DepositMethodID,Currency"
                )
            ...
            self.raise_exception_if_not_in(
                "Side", ("1", "2")
            )  # Only BUY and SELL sides

            self.raise_exception_if_required_tag_is_missing("OrdType")

            self.raise_exception_if_not_in(
                "OrdType", ("1", "2", "3", "4", "P")
            )  # market, limited, stop market, stop limited and pegged order

            if self.get("OrdType") == "2":
                self.raise_exception_if_required_tag_is_missing(
                    "Price"
                )  # price is required for limited orders
            ...
            self.raise_exception_if_length_is_greater_than(
                "StatusList", len([0, 1, 2, 4, 8])
            ...
            self.raise_exception_if_not_in(
                "Action", ["CONFIRM", "CANCEL", "PROGRESS", "COMPLETE", "MATCH"]
            )
            ...
            self.raise_exception_if_required_tag_is_missing("Action")
            self.raise_exception_if_not_in(
                "Action", ["CANCEL", "PROGRESS", "COMPLETE", "MATCH"]
            )
            ...
            self.raise_exception_if_required_tag_is_missing(
                "PartyBrokerSettlementAccount"
            )
            self.raise_exception_if_required_tag_is_missing(
                "CounterPartyBrokerSettlementAccount"
            )
    ...
    def set(self, attr, value):
        ...
        self.raw_message = json.dumps(
            dict(list(self.message.items()) + list({"MsgType": self.type}.items()))
        )
        return self
